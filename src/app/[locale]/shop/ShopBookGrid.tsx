...
export default function ShopBookGrid({ selectedGenres, selectedPriceRanges, currentPage, booksPerPage, setCurrentPage }: Props) {
        ...
        >
          ...
          <div className="p-4">
            ...
            {book.availableFor.includes('sell') ? (
              <p className="mt-1 text-lg font-semibold text-[#a8775a]">${book.price?.toFixed(2)}</p>
            ) : (<p className="mt-1 text-lg pt-7 font-semibold text-[#a8775a]"></p>)}
            <div className='flex items-center mt-4 justify-between'>
              {book.availableFor.includes('sell') && (
                <button onClick={(e) => e.stopPropagation} className="bg-btn-color text-[15px] hover:bg-[#a16950] text-gray-50 py-2 px-4 rounded-full transition duration-300">
                  Add to Cart
                </button>
              )}
